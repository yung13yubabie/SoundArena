...
import { castMatchVote } from "./actions";
...
export function MatchVoteList({
  ...
  const vote = (matchId: string, chosenRegistrationId: string) => {
    setError(null);
    startTransition(async () => {
      const result = await castMatchVote(matchId, chosenRegistrationId);
      if ("error" in result) {
        setError(result.error);
      } else {
        setVotedByMatch((prev) => ({ ...prev, [matchId]: chosenRegistrationId }));
      }
    });
  };
          ...
          return (
            <div key={m.matchId} className="glass p-4.5">
              ...
              <div className="grid grid-cols-1 items-center gap-3 sm:grid-cols-[1fr_auto_1fr]">
                {[m.a, m.b].map((side) => (
                  <div key={side.registrationId} className="rounded-[10px] border border-panel-border bg-white/[0.02] p-3.5">
                      ...
                      {side.isOwn ? (
                        ...
                        canVote && (
                          <button
                            ...
                            onClick={() => vote(m.matchId, side.registrationId)}
              ...
              {(m.a.isOwn || m.b.isOwn) && <p className="mt-2.5 text-[11.5px] text-ink-faint">你是這場的參賽者之一,不能投這一場</p>}
            </div>
          );
        })}
      </div>
