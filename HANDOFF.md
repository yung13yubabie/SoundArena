...
使用者在這輪對話丟出一大批新想法,已收斂成清單並排序,詳見下一節。

## 08-23:評分機制第一批——`external_vote` 補真邏輯、移除死選項 `video_traffic`

使用者確認要開始整理評分機制,依「小地基先打穩」原則,先盤點目前範本。細節見 [ADR-0042](docs/adr/0042-external-vote-and-remove-video-traffic.md)。

盤點 12 個範本時發現 `external_vote`(外部投票)跟 `video_traffic`(影片流量)從建置以來都沒有真正的計分邏輯,兩者都落在 `get_round_scores()` 的通用 else 分支(只能評審手動輸入,跟一般加分項目沒兩樣)。跟使用者確認定義後:`external_vote` 補上「只算沒有在這場比賽報名過的登入使用者的票」;`video_traffic` 直接移除(SoundArena 沒有任何影片功能)。

移除 `video_traffic` 第一次被外鍵擋下——使用者自己的「好友測試賽」測試比賽真的用過(兩筆計分項目),確認是可以清掉的測試資料後一併刪除。**手動檢查前端後追加抓到**:`create_competition_full()`(新比賽建立的真正路徑)跟 `DEFAULT_SCORE_ITEMS`(每輪獨立評分規則預設值)都各自寫死引用同一個 key,不修的話每個新比賽都會產生 `template_id=null` 的孤兒計分項目——同一批一起修掉,權重併進 external_vote(投票 40% + 外部投票 60%)。

真實 PoC 兩輪(3/3 + 5/5,對正式 Supabase 環境)+ `security-regression.mjs` 新增 3 項長期守護,33/33 通過。`tsc`/`eslint`/`build` 全程乾淨。

### 下一步:待處理清單(使用者已排序確認)

1. **SA-012 主動 alert + 訊息優化**——排進來了。追加需求:後台加一個 UI 欄位,讓非平台管理員的主辦人能透過 DC bot 聯繫到平台管理員(避免找不到人);alert 範圍要涵蓋「尚未投稿」「投票」等提醒規則類型,跟項目 3(後台已投稿/未投稿名單 + mail/DC 提醒)整合。
2. 賽制細節——使用者澄清:賽制積木系統(FormatBlock)骨架本來就有、後台看得到,但雙敗淘汰/復活賽/組隊賽亂數分組(後台可覆蓋)/月週期累積制/循環賽的實際遊戲邏輯還沒一一做出來,是「填細節」不是「從零蓋」。
3. 後台已投稿/未投稿名單 + mail/DC 提醒——OK,確認要做,可以直接用剛做好的 SA-005 架構。
4. 投稿頁列出詳細時程表 + 評分準則——OK,確認要做(報名頁已經有時程表了,這個是投稿頁也要加)。
5. Suno 連結自動爬資料 / 讀取 Suno 匯出檔案 meta 自動帶入——OK,確認要做(可行性還沒研究)。
6. 首頁動態效果(參考 bilibili.com/stableaudio.com)——OK,確認要做。
7. 首頁補主辦人資訊、賽事歷史——OK,確認要做。

SA-005 的 email 管道實際寄信驗證、「建立比賽自動接入 Discord 討論頻道」也還沒排進去,不在這輪確認的清單內。使用者要求「按照你(Claude)決定優先順序來」且「務必小模板根基建立好逐步向外」——下一步從第 1 項(SA-012)開始,先查證後台/DC bot 現有架構再動手。

## 08-23:SA-012——後台直接對參賽者發訊息 + 參賽者名單(第 1、3 項合併完成)

使用者澄清 SA-012 的真正需求:不是「其他主辦人聯繫平台管理員」,是「主辦人在後台直接對參賽者發 Discord/Email 訊息」,不透過我轉達。細節見 [ADR-0043](docs/adr/0043-sa012-organizer-message-to-participant.md)。

新增獨立的 `create_organizer_message_event()` RPC(不弱化 ADR-0015 鎖死內容的 `create_notification_event()`,兩支分開維護)——範圍收窄在:呼叫者要有這場比賽的 `review` 權限、對象必須是真實報名者、尊重參賽者自己的通知退訂設定、沒有支援的登入方式明確報錯。`/admin/review` 新增「參賽者名單」區塊,顯示已核准參賽者的投稿進度(X/Y 輪已投稿)+「傳訊息」按鈕——這個名單同時滿足待辦清單第 3 項(已投稿/未投稿名單),沒有拆成兩份分別做。

真實 PoC(7/7,含對使用者本人真實 Discord 帳號送出、確認收到)+ `security-regression.mjs` 新增 3 項,36/36 通過。`tsc`/`eslint`/`build` 全程乾淨。

### 下一步

按排序清單繼續第 2 項(賽制細節填空)。

**使用者回饋(採納,影響往後工作方式)**:(1)發送給使用者的訊息不能帶原始 UUID/工程相關內容——查證發現 `eventUrl()` 組的 `/submit?competition=<uuid>` 其實沒有任何作用(`/submit`/`/status` 兩個頁面根本沒讀這個 query string),直接拿掉(commit `1fce5a3`)。(2)多步驟工作以後全部做完再一次 commit/push/部署,不要每個小批次都個別跑一次 CI 讓使用者等。

## 08-23:賽制細節填空——動工前的根因查證

查證後確認一個影響範圍很大的事實:`format_blocks`(雙敗淘汰/循環賽/月週期累積制/組隊賽/敗部復活戰等)目前**純粹是標籤**,前後端都沒有任何程式碼依選了哪個 key 分支出不同行為——`grep` 整個 `web/src` 對這些 key 完全零命中。淘汰判定(`set_registration_eliminated()`)是主辦人手動點擊的 RPC,跟選了哪種賽制無關。也就是說「賽制細節還沒一一設立」不是小修小補,是要決定「這些格式到底要不要有真的自動化邏輯」這個更根本的問題,動手前必須先跟使用者確認範圍。

多輪 grilling 確認設計後(選「直接做自動化邏輯」、傳統兩兩對戰式循環賽、組隊賽用亂數分組+報名截止或前一輪結束自動觸發+後台可換組+單一合作作品),開始實作賽制細節的第一塊小地基:組隊賽亂數分組。

## 08-23:賽制細節填空第一批——組隊賽亂數分組(團隊分組小地基)

按「小地基先打穩,逐步向外」的原則,從賽制積木裡最小的一塊(組隊賽)開始做真的自動化邏輯。細節見 [ADR-0044](docs/adr/0044-team-lottery-grouping.md)。

grilling 過程中使用者兩次糾正我的初版提案:(1)循環賽不能重用現有排名邏輯偷懶,要做真的傳統兩兩對戰式循環賽(這批不涉及,留給下一批);(2)組隊分組的「前一輪結束」訊號不能用 `voting_closes_at`,因為投票截止到主辦人真的標完淘汰名單之間有空窗期,可能把即將被淘汰的人也分進下一輪隊伍——改成新增明確的「確認本輪結果」動作(`results_finalized_at` + `finalize_round_results()` RPC),主辦人在 `/judge` 頁面看過淘汰標記無誤後手動按一次。

**核心機制**:隊伍綁在輪次上(不是整場比賽),`form_team_groups_for_round()` 冪等、自我驗證觸發條件(第一輪看報名截止、非第一輪看前一輪確認結果),沒有天然使用者動作可以掛,設計成「造訪 `/status`、`/admin/format` 頁面時順便檢查」的 lazy check,`finalize_round_results()` 成功後額外立即嘗試一次,每日 cron 兜底。分組完成發 `team_assigned` 通知(隊名+隊友),`/admin/format` 選了「隊伍賽」時可設定每隊人數、看目前分組名單、用下拉選單手動換組。

**部署後才發現的真實設計錯誤**:第一版判斷式要求一輪同時掛 `team` 跟 `lottery` 兩個積木才觸發分組,但對照 `AdminFormatClient.tsx` 才發現 `grouping` 這個 category 在真實 UI 是單選(選了新的會把同 category 舊的刪掉),兩者不可能同時存在——照原判斷式整個功能在真實 UI 底下永遠不會觸發。改成只檢查 `team` 積木存在,額外一輪 PoC 驗證修正正確。

真實 PoC 兩輪(對正式 Supabase 環境,含對使用者本人真實 Discord 帳號送出分組通知、確認收到)+ `security-regression.mjs` 新增 8 項長期守護,44/44 通過。`tsc`/`eslint`/`build` 全程乾淨。

**誠實記錄**:新增的三個 UI 介面(`/judge` 的「確認本輪結果」按鈕、`/admin/format` 的每隊人數設定與換組面板)沒有經過瀏覽器肉眼驗證——沒有既有登入 session,OAuth 登入需要使用者本人操作。`team_size` 未設定時預設 3 人是我單方面提出、未經使用者逐字確認的軟性假設。

### 下一步

團隊「單一合作作品」的投稿流程(讓隊伍共用一筆投稿而非各自投稿)是下一批,這批只做到分組本身。之後依序:雙敗淘汰(含復活賽)、循環賽(傳統兩兩對戰,需要獨立設計)、月度/週期累積制。淘汰配額是否依報名人數比例自動調整——使用者已提出但明確定調「等報名時間結束後再來討論」,不是這批的範圍。

## 08-23:向使用者回報整體進度後,收到修正——淘汰配額自動化(通用機制)+ 組隊分組改成「填組數、均分打散」

使用者要求完整進度報告(整體完成度、距離上線多久、還缺什麼),回報過程中順便測了 Email 通知,發現一個新問題:Resend 帳號是用 `u03227010@gmail.com` 註冊,沒驗證網域前只能寄給帳號自己的信箱,寄不到任何真實參賽者——這代表 Email 管道在買網域驗證之前,對真實使用者完全不能用,不是「還沒測」而是技術上寄不出去。也順手說明了 SA-011(email/密碼註冊後門)風險的具體攻擊面(繞過 OAuth 灌小號投票),建議使用者自己去 Supabase 後台關掉,不需要曝露 OAuth 密鑰。

使用者對回報裡「組隊分組每隊人數預設3人」提出修正,經過多輪 grilling 收斂出一個比原計畫更通用的機制,細節見 [ADR-0045](docs/adr/0045-elimination-quota-and-group-count.md):

- **分組演算法改版**:「每隊固定人數」換成「主辦人填這輪要分幾組(上限5),系統均勻分配剩餘人數,餘數打散到前面幾組」(14人分3組→5、5、4,不是4+4+4+2)。
- **淘汰配額自動化(全新,通用機制)**:每輪填一個「淘汰百分比」,套用在該輪即時的在賽人數上(不是報名截止當下的原始總數)——「確認本輪結果」按下去時,系統依這輪即時分數,自動淘汰墊底的那個百分比人數。這個機制**不綁定任何特定賽制積木**,單敗/雙敗淘汰、循環賽、月/週期累積制之後都共用同一套,大幅縮小賽制細節填空剩餘的工程量。這推翻了最初建置 `/judge` 時「淘汰必須人工點選」的實作理由(SPEC.md 本身沒有明文規定這點,查證過)。

實作上抽出新的 `web/src/lib/judgeScoring.ts`(`getJudgeScoringData()`),把原本寫死在 `judge/page.tsx` 的評分資料組裝邏輯抽成共用函式,`finalizeRoundResults()` 才能重用同一份資料算淘汰名單——不能沿用 `get_round_scores()`/`get_round_submissions()`,那兩個只有「比賽公開+投票已截止」才回傳資料,是給公開結果頁用的。`finalize_round_results()` RPC 加了冪等保護(已確認過的輪次不能重算)。`/judge` 既有的手動淘汰按鈕保留,跟自動機制自然疊加(手動當例外覆蓋,自動補足剩下的百分比)。

真實 PoC(11/11)+ `security-regression.mjs` 新增/修改 10 項,48/48 通過。`tsc`/`eslint`/`build` 全程乾淨。UI(`/admin/format` 的分組數設定、自動淘汰面板;`/judge` 的淘汰預估文案)未經瀏覽器肉眼驗證。組數上限未設定時預設 2 組——同 ADR-0044 的 team_size=3,是未經使用者逐字確認的軟性假設。

### 下一步

使用者確認「建立比賽自動開 Discord 頻道」+「報名後自動加入對應頻道」這個功能要做,是完全獨立的下一批,還沒開始。之後才輪到雙敗淘汰/循環賽/月週期累積制(現在都已經共用淘汰配額機制,可能只需要少量 UI/標籤層面的工作,不用重寫淘汰邏輯)。SPEC.md 第6節「該輪淘汰/晉級結果」通知事件類型還沒實作,之後做的話要記得套用「發送前需人工確認」這條規則。

**使用者修正(重要,影響賽制系統的理解)**:單敗/雙敗淘汰其實是要「兩兩對戰取勝」(個人或隊伍都適用),跟這批做的「排名墊底幾%自動淘汰」是完全不同的機制——上一節 ADR-0045 說這套自動淘汰是「通用機制,涵蓋所有賽制標籤」對單雙敗淘汰是不對的,那兩個之後要單獨設計對戰配對邏輯(誰跟誰配對、勝負怎麼判定、雙敗的敗部怎麼運作)。月/週期累積制確認還是吃這套%淘汰機制,但「30%配2次週期」的意思是整場合計淘汰30%,不是每個週期各自扣30%——這個細節等真的做到那個賽制再跟使用者細談。

## 08-23:Discord 頻道系統——建立比賽自動開私人頻道 + 報名自動加入

使用者確認要做,細節見 [ADR-0046](docs/adr/0046-discord-competition-channel.md)。沿用既有 SA-005 的 Discord 基礎設施(同一個 Bot、同一個伺服器),不開新伺服器。建立比賽時開一個私人文字頻道(對 `@everyone` deny 可見,只有主辦人跟報名者透過頻道層級權限覆寫個別開放),`competitions.discord_channel_id` 比照 `profiles.discord_user_id` 先例只給 service_role 寫。

動手前先查證 Bot 目前在伺服器裡的實際權限(不是假設)——發現 Bot 有「管理頻道」但沒有「管理身分組」,後者才是對頻道做「特定成員」權限覆寫需要的權限。請使用者直接在 Discord 伺服器設定裡幫 Bot 的身分組加開這個權限,不需要重新邀請、不需要走 OAuth。真實 PoC 過程中還抓到一個坑:第一版建立頻道只設定「@everyone 看不到」,沒有同時允許 Bot 自己看得到,結果 Bot 建完頻道自己讀寫都被 403 擋下——修成建立頻道的同一次 API 呼叫裡順便把 Bot 自己也加進權限覆寫。

真實 PoC(9/9,對正式 Supabase 環境 + 真實 Discord 伺服器,含打 API 直接驗證使用者本人的真實帳號被授予存取權、測試頻道跑完即刪)+ `security-regression.mjs` 新增 1 項,49/49 通過。`tsc`/`eslint`/`build` 全程乾淨。

**尚未驗證**:沒有第二個真實 Discord 帳號,報名觸發的頻道加入邏輯只驗證到資料庫層面的前提條件,底層 API 函式已經在主辦人自動授權那步驗證過對真實帳號有效。
...
## 08-23:單敗淘汰(single_elimination)

使用者確認接著做單/雙敗淘汰,`mattpocock-skills:grilling` 跑三輪收斂,先做單敗、雙敗留下一批。細節見 [ADR-0049](docs/adr/0049-single-elimination.md)。

**設計**:只有個人對戰;第一輪隨機配對,**每一輪都重新隨機配對**(不保留固定賽事樹位置,允許同一組對手不同輪次又碰到,使用者確認可接受);人數是奇數時隨機抽中一人自動輪空晉級;橫跨好幾輪比賽用的「輪次」,主辦人自己陸續加,不用事先算好總共幾輪,系統看前一輪確認結果自動配對下一輪;沿用循環賽的 `match_votes` 決定輸贏;**平手時整個拒絕確認本輪結果**(不能像循環賽算平局),列出哪些場次平手,請主辦人到既有的「本輪專屬時程」延長投票時間;**不接** `elimination_percent`,輸家 100% 出局。

沿用循環賽剛建的 `matches`/`match_votes`,只把 `matches.pool_id` 放寬成可為 null(單敗淘汰不需要分池概念)。`/admin/format` 新增 `SingleEliminationPanel`(取代 `EliminationPercentPanel`,因為那個欄位對這種賽制沒意義),`/vote` 的配對投票偵測擴大成同時認循環賽跟單敗淘汰,共用 `MatchVoteList`。

真實 PoC(5人配對含輪空、平手正確拒絕確認、補票後正確結算、確認結果100%淘汰輸家、下一輪正確觸發),9/9 通過。`security-regression.mjs` 新增 6 項。`tsc`/`eslint`/`build` 全程乾淨。**尚未驗證**:新 UI 沒有瀏覽器肉眼驗證過。

**賽制細節填空到此全部完成**(組隊賽亂數分組、月/週期累積制、循環賽、單敗淘汰),只剩雙敗淘汰(敗部規則)待定,還沒開始設計。

## 08-23:雙敗淘汰(double_elimination)

使用者確認繼續做,`mattpocock-skills:grilling` 跑三輪收斂設計。細節見 [ADR-0050](docs/adr/0050-double-elimination.md)。

**設計**:只有個人對戰;每輪 0敗組(勝部)跟1敗組(敗部)**分開隨機配對**,各組人數是奇數時該組自己抽一人輪空;勝部輸的人**不淘汰**(敗場數變1,下一輪歸進敗部組),敗部輸的人是第二次輸**直接淘汰**;敗場數不存欄位,即時查詢這場比賽底下所有雙敗淘汰輪次的對戰紀錄算出來;最終戰(0敗組剩1人、1敗組剩1人)**不做傳統「敗部水漲」重賽規則**,誰贏誰就是冠軍(簡化,使用者確認);平手處理、投票機制沿用單敗淘汰同一套;不接 `elimination_percent`。

**自己抓到並修好的死結 bug**:寫 migration 時發現「兩組各自配對」的規則在最終戰場景(0敗組1人+1敗組1人,兩組各自都不滿2人)會產生0場次卡死,推送前就加了特別判斷插入跨組 `bracket='final'` 場次。`security-regression.mjs` 驗證這條路徑確實生效(見下方)。

沿用循環賽/單敗淘汰的 `matches`/`match_votes`,新增 `matches.bracket` 欄位(`'winners'|'losers'|'final'`)決定輸家該不該被淘汰。`/admin/format` 新增 `DoubleEliminationPanel`(取代 `EliminationPercentPanel`),依 `bracket` 分「勝部/敗部/最終戰」顯示配對。`/vote` 配對投票偵測擴大成同時認循環賽、單敗淘汰、雙敗淘汰三種。

真實 PoC(平手正確擋下整輪、勝部/敗部/最終戰三場都決出勝負時淘汰名單正確只含敗部+最終戰輸家),3/3 通過。`security-regression.mjs` 新增 9 項(3人跑完整4輪敘事:奇數配對含輪空、冪等、陌生人擋下、勝部輸家不淘汰、0敗組/1敗組分組正確、敗部輸家真的被淘汰、最終戰死結 bug 修法確實生效),70/70 全部通過。`tsc`/`eslint`/`build` 全程乾淨。**尚未驗證**:新 UI 沒有瀏覽器肉眼驗證過。

**賽制細節填空原本規劃的五種賽制全部完成**(組隊賽亂數分組、月/週期累積制、循環賽、單敗淘汰、雙敗淘汰)。下一步:瀏覽器肉眼驗證這整批(含前幾批:團隊分組、淘汰配額、Discord 頻道、月/週期累積制、循環賽、單敗淘汰、雙敗淘汰)所有新增的 UI 畫面,目前全部都只驗證到資料庫/RPC 層,沒有實際點過畫面。

## 08-27:移除「業界導師制」(mentor_system)純標籤

使用者盤點「特殊機制」分類裡還有哪些是純標籤沒接上邏輯,查證確認 `wildcard_revival`(敗部復活戰)跟 `mentor_system`(業界導師制)兩個完全沒有任何程式碼引用(含 SQL),`themed_round`(限定主題輪)則已經有真的邏輯(`/submit` 頁面會顯示設定的主題給投稿者)。使用者要求業界導師制直接整個移除。

`format_blocks` 目錄表是動態查表(`/admin/format` 特殊機制清單直接查這張表渲染,不是寫死在前端),移除只需要一個 migration:刪掉 `mentor_system` 這筆目錄列(先防禦性清掉可能存在的 `round_format_blocks` 掛載,理論上應該是0筆)。推送後直接查表確認已經從目錄消失,`/admin/format` 不需要任何程式碼異動就會自動少一個選項。

### 下一步

使用者接著問「敗部先還是限定先」——回報建議:敗部復活戰(`wildcard_revival`)是真正還沒設計的空殼,限定主題輪目前的顯示型範圍已經算完整(自動驗證投稿是否符合主題不現實,留給評審人工判斷比較合理),所以建議先做敗部復活戰。附帶提醒一個命名/概念上的潛在混淆:`double_elimination` 的目錄標籤本身就寫「雙敗淘汰(含敗部復活)」——也就是說敗部本身的「輸一場還有機會」已經是雙敗淘汰內建的行為。所以 `wildcard_revival` 這個獨立特殊機制積木要設計的,應該是**另一種**跟雙敗淘汰不同的復活方式(例如:單敗淘汰/循環賽這種沒有內建復活機制的賽制,額外抽一位已淘汰者用外卡/投票方式重新加入),真正動工前需要先用 grilling 把這個範圍釐清,不能假設使用者要的就是雙敗淘汰那一套。

## 08-27:外卡復活戰(wildcard_revival)

使用者確認方向是「外卡復活」(排除雙敗淘汰、只限最近一輪被淘汰者、觀眾投票、整場限用一次),`mattpocock-skills:grilling` 跑四輪收斂設計。細節見 [ADR-0051](docs/adr/0051-wildcard-revival.md)。

**設計**:適用單敗淘汰/循環賽/月週期累積制(排除雙敗淘汰,理由見上一則);候選資格只限「觸發當下最近一次確認結果的那一輪」被淘汰者,前 N 名(離晉級線最近,N 主辦人自訂)——循環賽用勝場數排、單敗淘汰沒有分數可排改用場次票數差距排;開放觀眾投票,候選人顯示被淘汰那輪的投稿內容並沿用原輪匿名設定;整場比賽限用一次;主辦人手動開啟投票視窗(比照本輪專屬時程);**時間窗限制**(使用者確認可接受):只能在「某輪確認結果之後、下一輪配對正式產生之前」開啟,錯過這次還有下次機會(但整場只有一次)。

新建三張表(`wildcard_revival_events`/`_candidates`/`_votes`,不跟 `matches`/`match_votes` 共用——候選人是已淘汰者,語意不同):`unique(competition_id)` 在資料庫層面直接保證整場限用一次;候選名單開啟當下就算好寫死,不是投票結束時才重算。`open_wildcard_revival_event()` RPC 驗證候選人真的是那輪淘汰的人、下一輪還沒產生分組/配對(查 `teams`/`pools`/`matches` 任一張表)才允許開啟;`resolve_wildcard_revival_event()` 套用贏家、把狀態改回 active;候選名單排序演算法在 TS 層算(`lib/wildcardRevival.ts` 的 `computeWildcardRevivalCandidates()`),RPC 只驗證不重算,避免 SQL/TS 兩邊漂移。`/admin/format` 新增競賽層級的 `WildcardRevivalPanel`(不是輪次層級),新增獨立路由 `/vote/wildcard?event=<id>`。

真實 PoC(循環賽勝場數排序、單敗淘汰票數差距排序、平手偵測、補票打破平手),4/4 通過(過程中發現自己在 PoC 手算勝場數時算錯,修正測試期望值後複跑通過,不是演算法本身的 bug)。`security-regression.mjs` 新增 12 項(RPC 邊界:權限、候選人驗證、整場限用一次、自己不能投自己、投給非候選人拒絕、投票時窗驗證、確認結果套用回 active、下一輪配對已產生時擋下開啟),82/82 全部通過。`tsc`/`eslint`/`build` 全程乾淨。**尚未驗證**:新 UI(`WildcardRevivalPanel`、`/vote/wildcard`)沒有經過瀏覽器肉眼驗證。

**未涵蓋**:沒有「主辦人取消已開啟投票」的功能,一旦開啟只能等截止確認或延長時間,目前沒有需求先不做。
...
## 08-28:Codex 第二輪對抗式審查——孤兒節點+相容性缺口,6項發現全部修復

使用者要求繼續派 `codex:codex-rescue` 做更嚴格的第二輪,專門獵孤兒節點。第一次嘗試沙盒連讀檔案都被拒絕,Codex 誠實回報「發現數0,是環境問題不是專案沒漏洞」,沒有捏造。重新派工並明確要求讀取權限後第三次成功,交出6項發現(3高2中1低)。過程中使用者親自登入,我接手用真實瀏覽器 session 操作 `/admin/format`,獨立發現「隊伍賽疊加淘汰賽制配對邏輯不認隊伍」——跟 Codex 的發現完全一致,兩邊獨立驗證交叉確認。細節見 [ADR-0053](docs/adr/0053-codex-second-pass-orphan-fixes.md)。

**Finding 1(高)**:已產生真實賽程資料(teams/pools/matches)或已確認結果的輪次,`toggleFormatBlock()` 原本可以任意切換淘汰方式/分組方式,導致舊資料按新規則被誤判(例如切成雙敗淘汰後舊場次沒有bracket欄位,結算判斷成沒人該淘汰)。

**Finding 2(高,我瀏覽器實測+Codex獨立確認一致)**:隊伍賽疊加單敗/雙敗淘汰/循環賽,配對邏輯完全用個人報名為單位,組好的隊伍純裝飾。在真正支援隊伍為對戰單位前,直接禁止這個組合(月/週期累積制不是對戰配對賽制,不受限制)。

**Finding 3(高,對應使用者提到的評分問題)**:月/週期累積制允許各輪覆寫評分規則,會讓合併計分的欄位對不上、自動淘汰跟外卡候選排序靜默算錯。雙向鎖死:週期累積制輪次不能開獨立評分規則,已有獨立評分規則的輪次不能切成週期累積制。

**Finding 4(中)**:循環賽沒選抽籤分組時配對函式靜默產生0場次沒有任何錯誤訊息。改成要求round_robin必須先有lottery grouping,設計成「先選抽籤分組、再選循環賽」的順序永遠走得通,不會卡死使用者。

**Finding 5(中,我獨立驗證確認,也是我自己曾經被誤導的地方)**:「敗部復活戰」round層級勾選框是孤兒標籤——真正的外卡復活是競賽層級獨立區塊,完全不看這個勾選框,勾了沒用不勾也正常運作,比純標籤更容易誤導主辦人。直接移除,比照業界導師制的做法。

**Finding 6(低)**:`MatchRow.pool_id` 型別跟schema不一致,靠不安全cast掩蓋,已修正為 `string | null`。

新增 `set_round_format_block()` RPC 取代 `toggleFormatBlock()` 對 round_format_blocks 的直接table操作,單一交易內做完所有驗證,避免Server Action分開刪除+插入兩次呼叫之間的中間態風險。
